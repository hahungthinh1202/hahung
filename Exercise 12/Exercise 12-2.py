...
location = requests.get(f"http://api.openweathermap.org/geo/1.0/direct?q={city_name}&limit=5&appid=dd4cee5c601907afe720b23de645a88f")
# After request the location, it will return a list of possible place, I choose the first one in the list
# Then using those location to make another request for the actual weather
weather = requests.get(f"https://api.openweathermap.org/data/3.0/onecall?lat={location.json()[0]['lat']}&lon={location.json()[0]['lon']}&appid=dd4cee5c601907afe720b23de645a88f")
